...
dnew <- rep(0,n)
for (m in 2:nit) {
  pvect <- pi*dnorm(y,mu1[m-1],sqrt(1/lambda[m-1]))/
    (pi*dnorm(y,mu1[m-1],sqrt(1/lambda[m-1]))+(1-pi)*dnorm(y,mu2[m-1],sqrt(1/lambda[m-1])))  
  ...
  mu1.new <- rnorm(1,(lambda[m-1]*sumx1)/(0.01+n1*lambda[m-1]),sqrt(1/(0.01+n1*lambda[m-1])))
  mu2.new <- rnorm(1,(lambda[m-1]*sumx2)/(0.01+n2*lambda[m-1]),sqrt(1/(0.01+n2*lambda[m-1])))
  lambda.new <- rgamma(1,n/2+1,(1+0.5*(sum(y^2)-2*(mu1.new*sumx1+mu2.new*sumx2)
...
count=0
for (m in 1:nit) {
  fnew <- pi*dnorm(s,mu1[m],sqrt(1/lambda[m]))+(1-pi)*dnorm(s,mu2[m],sqrt(1/lambda[m]))
  density.est[,m] <- fnew
  count=count+1
}
loc <- 1:1000
...
count=0
for (m in 1:nit) {
  fnew1 <- pi*dnorm(s,mu1[m],sqrt(1/lambda[m]))+(1-pi)*dnorm(s,mu2[m],sqrt(1/lambda[m]))
